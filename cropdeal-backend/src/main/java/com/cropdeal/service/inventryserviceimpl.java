...
public class inventryserviceimpl implements inventryService {
	...
	public product addproduct(int formerId,product product) throws noProductFoundException {
		...
		List<product> prodtOptional=productRepositry.findByFarmerIdAndProductNameAndIsDeletedFalse(product.getFarmerId(), product.getProductName());
...
//		rabbitmqEmitter.emmitmsg(emmitmap);
		...
		String emailIdById = profileRepositry.findEmailIdById(formerId);
		if(StringUtils.isNotBlank(emailIdById))
			mailsenderservice.sendProductAddedMail(emailIdById,product);
	...
	public Object updateproduct(product product,int farmerId) throws noProductFoundException {
		List<product> prodtOptional=productRepositry.findByFarmerIdAndProductIdAndIsDeletedFalse(farmerId, product.getProductId());
		if(prodtOptional.size()==0) {
...
//			rabbitmqEmitter.emmitmsg(emmitmap);
			...
			String emailIdById = profileRepositry.findEmailIdById(product2.getFarmerId());
			if(StringUtils.isNotBlank(emailIdById))
				mailsenderservice.sendProductUpdatedMail(emailIdById,product);
			
			return productRepositry.save(new product(product2.getProductId(), product2.getFarmerId(), product.getProductName(), product.getAvailableQuantity(), product2.getInitialQuantity(), product.getDate(), product.getPrice(),product.getProductDetails() , product2.getStatus(), product.getProductImages(), product2.getReviews(),false));
		}
	}
	...
	public void deleteproduct(Integer id, int formerId) throws noProductFoundException {
		...
		List<product> prodtOptional=productRepositry.findByFarmerIdAndProductIdAndIsDeletedFalse(formerId, id);
...
//			rabbitmqEmitter.emmitmsg(emmitmap);
			...
			String emailIdById = profileRepositry.findEmailIdById(prodtOptional.get(0).getFarmerId());
			if(StringUtils.isNotBlank(emailIdById))
				mailsenderservice.sendProductRemovedMail(emailIdById, prodtOptional.get(0).getProductName());
	...
	public cart addtocart(int merchentId, Map<String, String> inputdata) throws noProductFoundException {
		...
		product product=getProductById(Integer.parseInt( inputdata.get("productId")));
		
		Optional<cart> cartitem=cartRepositry.findByProductProductIdAndMarchentId(Integer.parseInt( inputdata.get("productId")),merchentId);
	...
	public product getProductById(Integer productId) throws noProductFoundException {
		...
		List<product> prodtOptional=productRepositry.findByProductIdAndIsDeletedFalse(productId);
		if(prodtOptional.size()==0) {
	...
	public List<product> getallProductsByFarmerId(int farmerId) {
		...
		return productRepositry.findByFarmerIdAndIsDeletedFalse(farmerId);
	}

	@Override
	public void removefromCart(Integer productId,int merchentId) throws noProductFoundException {
	...
	@Override
	public reviews updatereview(Integer productId, reviews reviews,int dealerid) throws noProductFoundException {
...
//		productId
//		quantity
	    product products = productRepositry.findByProductIdAndIsDeletedFalse(Integer.parseInt(orderdetails.get("productId"))).get(0) ;
...
//	    System.out.println("Product ID: " + entry.getKey() + ", Quantity: " + entry.getValue());
			product products = productRepositry.findByProductIdAndIsDeletedFalse(Integer.parseInt(entry.getKey())).get(0);
...
//		    System.out.println("Product ID: " + entry.getKey() + ", Quantity: " + entry.getValue());
	...
	@Override
	public double getavgRatingOfProduct(Integer productId) {
